...
//             q[j] = center + (u < 0.5 ? 1 : -1) * range * (1.0 - std::sqrt(v)) / 2.0;
//         }
//         attempts++;
//         if (attempts >= max_attempts) {
//             for (size_t j = 0; j < 6; j++) {
//                 q[j] = (joint_limits_max[j] + joint_limits_min[j]) / 2.0;
//             }
//             break;
//         }
...
//     // Calculate direction vector with proper angle wrapping
...
//     // Always use step_size (no scaling near goal)
//     double scale = std::min(1.0, step_size / total_dist);
...
//     if (path.size() <= 2) return;  // Nothing to optimize
    
//     std::cout << "Starting path optimization. Initial size: " << path.size() << std::endl;
    
//     const double max_step = step_size * 1.2;  // Maximum allowable step size
//     bool made_changes;
...
//         while (i < path.size() - 2) {
//             // Try to remove node i+1 by connecting i to i+2
//             double potential_step = 0.0;
//             for (size_t j = 0; j < 6; ++j) {
//                 double diff = path[i+2]->q[j] - path[i]->q[j];
//                 if (j >= 3) {  // For rotation joints
//                     while (diff > M_PI) diff -= 2 * M_PI;
//                     while (diff < -M_PI) diff += 2 * M_PI;
//                 }
//                 potential_step += diff * diff;
//             }
//             potential_step = std::sqrt(potential_step);
            
//             if (potential_step <= max_step) {
//                 auto [collision_free, _] = isCollisionFree(path[i], path[i+2]);
//                 if (collision_free) {
//                     // Remove the intermediate node
//                     path.erase(path.begin() + i + 1);
//                     made_changes = true;
//                     continue;  // Don't increment i, check the new i+1 node
//                 }
//             }
//             i++;  // Only increment if we couldn't remove a node
//         }
//     } while (made_changes);
    
//     std::cout << "Path size after optimization: " << path.size() << std::endl;
...
//         if (dist > max_step) {
//             std::cout << "Warning: Large step detected between nodes " << i-1 << " and " << i 
//                       << ": " << dist << std::endl;
            
//             // Fix by inserting intermediate nodes
...
//                 auto* new_node = new Node(interpolated_q);
...
//             i += new_nodes.size();  // Skip past newly inserted nodes
//         }
//     }
    
//     std::cout << "Final path size after step size enforcement: " << path.size() << std::endl;
// }
...
//     // Plot obstacles - scale them down to match path scale
//     for (const auto& obstacle : obstacles) {
//         // Scale factor to bring obstacles into view
//         const double scale = 0.05; // This will bring 200-250 range down to 10-12.5 range
...
//         std::map<std::string, std::string> obstacle_properties;
//         obstacle_properties["color"] = "blue";
//         obstacle_properties["linestyle"] = "--";
//         obstacle_properties["label"] = "Obstacle";
        
//         plt::plot3(obs_x, obs_y, obs_z, obstacle_properties);
//     }

//     // Plot the path
//     plt::plot3(x, y, z, path_properties);
//     plt::legend();
//     plt::show();
// }
...
//         path_properties["color"] = "red";
//         path_properties["linestyle"] = "-";
//         path_properties["linewidth"] = "2";
//         path_properties["label"] = "RRT* Path";

//         plt::figure(1);  // Explicitly numbered figure
...
//         obstacle_properties["color"] = "blue";
//         obstacle_properties["linestyle"] = "--";
//         obstacle_properties["linewidth"] = "2";
//         obstacle_properties["label"] = "Obstacle";

//         plt::figure(2);  // Different figure number
//         plt::plot3(obs_x, obs_y, obs_z, obstacle_properties);
//         plt::xlabel("X");
//         plt::ylabel("Y");
//         plt::set_zlabel("Z");
//         plt::title("Obstacle Location");
//         plt::legend();
//         plt::show();
//     }
// }
...
//     // Set labels and show single plot
//     plt::xlabel("X");
//     plt::ylabel("Y");
//     plt::set_zlabel("Z");
...
//     // Scale obstacle coordinates
//     const double scale = 0.05;
//     const auto& obstacle = obstacles[0];  // Just plot first obstacle for now
//     std::vector<double> obs_x = {obstacle.min_point[0] * scale, obstacle.max_point[0] * scale};
//     std::vector<double> obs_y = {obstacle.min_point[1] * scale, obstacle.max_point[1] * scale};
//     std::vector<double> obs_z = {obstacle.min_point[2] * scale, obstacle.max_point[2] * scale};
...
//     // Plot path and obstacle together
//     std::map<std::string, std::string> path_properties;
//     path_properties["color"] = "red";
//     path_properties["linestyle"] = "-";
//     path_properties["label"] = "Path";
